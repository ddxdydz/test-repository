...
class ShiftPacker(Packer):
    ...
    @staticmethod
    def _untamp_array_by_shift(tamped_array: np.ndarray, bit_mask: np.array,
                               values_per_dtype: int, shifts: np.array) -> np.ndarray:
    ...
    def pack_array(self, array: np.ndarray) -> bytes:
        ...
        if self.bits_per_value == 1:
            ...
            shifts = ShiftPacker._SHIFTS_MAP[self.bits_per_value]
            packed_array = self._tamp_array_by_shift(flatten_array, dtype, values_per_dtype, shifts).tobytes()
        else:
    ...
    def unpack_array(self, data: bytes) -> np.ndarray:
        ...
        if self.bits_per_value == 1:
            ...
            dtype, dtype_bits_count, values_per_dtype = ShiftPacker._TYPES_MAP[self.bits_per_value]
            shifts = ShiftPacker._SHIFTS_MAP[self.bits_per_value]
            bit_mask = ShiftPacker._MASK_MAP[self.bits_per_value]
            tamped_array = np.frombuffer(packed_array, dtype=dtype)
            array = self._untamp_array_by_shift(
                tamped_array, bit_mask, values_per_dtype, shifts)[:expected_size].reshape(shape)
        else:
